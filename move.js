 ...
 */
//move需要传3个参数：curEle target  duration
//                       {left:1200,top:400}
(function(){
    var zhufengEffect={
    ...
    function move(curEle,target,duration,effect,callback){
        ...
        //Swiper.根据用户传的数字来实现不同的运动；
        if(typeof effect==='number'){
            var str=ary[effect%ary.length];//"Elastic-easeOut"
            ary=str.split('-');
            tmpEffect=ary.length>=2?zhufengEffect[ary[0]][ary[1]]:zhufengEffect[ary[0]];//zhufengEffect.Elastic.easeOut
